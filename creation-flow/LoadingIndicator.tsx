...
const LoadingIndicator: React.FC<LoadingIndicatorProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 bg-gray-800/50 rounded-lg border border-gray-700">
      ...
      <h2 className="text-2xl font-semibold mt-8 text-gray-200">{title}</h2>
      <p className="mt-2 text-gray-400 text-center transition-opacity duration-500">
        {loadingMessages[messageIndex]}
      </p>
      {modelName && (
        <div className="mt-6 text-xs text-white/30 font-mono tracking-wider uppercase bg-white/5 px-3 py-1.5 rounded-full border border-white/5">
          Model: {modelName}
        </div>
      )}
    </div>
